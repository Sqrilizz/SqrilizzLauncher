    ...
    app.opener().reveal_item_in_dir(path).map_err(|e| {
        ...
        TheseusSerializableError::Theseus(
            theseus::ErrorKind::OtherError(format!(
                "Failed to highlight file in folder: {}",
                e
            ))
            .into(),
        )
...
pub fn open_path<R: Runtime>(
    ...
    tracing::info!("open_path called with: {:?}", path);
    app.opener()
    ...
    tracing::info!("open_path completed successfully");
    Ok(true)
}

#[tauri::command]
pub fn show_launcher_logs_folder<R: Runtime>(
    app: tauri::AppHandle<R>,
) -> Result<bool> {
...
pub async fn get_opening_command(
    ...
    return if let Some(payload) = payload.as_ref() {
        ...
        tracing::info!("No startup command — continuing normal launcher mode");
        Ok(None)
    };
}
