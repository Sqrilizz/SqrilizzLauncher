...
#[tauri::command]
pub async fn profile_get_full_path(path: &str) -> Result<String> {
    tracing::info!("profile_get_full_path called with: {}", path);
    let res = profile::get_full_path(path).await?;
    let result = res.to_string_lossy().to_string();
    tracing::info!("profile_get_full_path returning: {}", result);
    Ok(result)
}
...
pub async fn profile_get_pack_export_candidates(
    ...
    tracing::info!(
        "profile_get_pack_export_candidates called with: {}",
        profile_path
    );
    let candidates = profile::get_pack_export_candidates(profile_path).await?;
    // Convert to Vec<String> for JSON serialization
    let result: Vec<String> =
        candidates.into_iter().map(|p| p.to_string()).collect();
    tracing::info!(
        "profile_get_pack_export_candidates returning {} items",
        result.len()
    );
    Ok(result)
}
