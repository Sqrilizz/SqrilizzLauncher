			...
			<ContentPageHeader>
				...
				<template #actions>
					<div class="flex gap-2">
						...
						<!-- [AR] Patch: Replaced OverflowMenu with simple buttons to fix crash -->
						<ButtonStyled size="large" circular>
							<button
								...
								:options="[
									{
										id: 'open-folder',
										action: () => {
											console.log('Opening folder:', instance.path)
											showProfileInFolder(instance.path)
												.then((result) => console.log('Folder opened successfully:', result))
												.catch((err) => {
										...
										action: () => {
											...
											try {
												if (exportModal.value) {
													...
													console.error('exportModal.value is null')
												}
											} catch (err) {
...
const openFolderClick = async () => {
	console.log('Opening folder:', instance.value.path)
	try {
		const result = await showProfileInFolder(instance.value.path)
