...
defineExpose({
	show: () => {
		console.log('[ExportModal] show() called')
		try {
			exportModal.value.show()
			console.log('[ExportModal] modal shown, calling initFiles()')
			initFiles()
		} catch (err) {
			console.error('[ExportModal] Error in show():', err)
			throw err
		}
	},
})
...
const initFiles = async () => {
	console.log('[ExportModal] initFiles() called')
	const newFolders = new Map()
	const sep = '/'
	files.value = []
	console.log('[ExportModal] Calling get_pack_export_candidates with:', props.instance.path)
	await get_pack_export_candidates(props.instance.path).then((filePaths) => {
		console.log('[ExportModal] get_pack_export_candidates returned:', filePaths)
		filePaths
			.map((folder) => ({
				path: folder,
...
// [AR] Patch: Don't call initFiles() on component mount - only when modal is shown
// await initFiles()
