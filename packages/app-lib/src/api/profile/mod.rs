...
pub async fn try_update_playtime(path: &str) -> crate::Result<()> {
    ...
    // [AR] Patch: Disable Modrinth analytics for offline launcher
    // Skip playtime submission to avoid 401 errors
    tracing::debug!("Playtime analytics disabled for offline launcher");

    // Update the profile metadata locally without submitting
