...
pub async fn set_world_display_status(
    ...
    tracing::debug!(
        "set_world_display_status called: instance={}, world_type={:?}, world_id={}, display_status={:?}",
        instance,
        world_type,
        world_id,
        display_status
    ...
    if instance.is_empty() {
        tracing::error!("Instance path is empty");
        return Err(crate::ErrorKind::InputError(
            "Instance path cannot be empty".to_string(),
        )
        .into());
    }
    if world_id.is_empty() {
        tracing::error!("World ID is empty");
        return Err(crate::ErrorKind::InputError(
            "World ID cannot be empty".to_string(),
        )
        .into());
    }

    let state = State::get().await?;
    match attached_world_data::set_display_status(
        instance,
        world_type,
        world_id,
        display_status,
    ...
    {
        Ok(_) => {
            tracing::debug!("set_world_display_status completed successfully");
            Ok(())
        }
        Err(e) => {
            tracing::error!(
                "Failed to set world display status: instance={}, world_type={:?}, world_id={}, error={}",
                instance,
                world_type,
                world_id,
                e
