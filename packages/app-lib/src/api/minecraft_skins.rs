...
/// Returns an empty vector if no online profile is available (offline mode).
#[tracing::instrument]
pub async fn get_available_capes() -> crate::Result<Vec<Cape>> {
    tracing::debug!("get_available_capes called");
    ...
    let selected_credentials =
        match Credentials::get_default_credential(&state.pool).await {
            Ok(Some(creds)) => creds,
            Ok(None) => {
                tracing::debug!("No credentials available");
                return Ok(Vec::new());
            }
            Err(e) => {
    ...
    let profile = match selected_credentials.online_profile().await {
        Some(profile) => profile,
        None => {
            tracing::debug!(
                "No online profile available, returning empty capes list"
            );
            return Ok(Vec::new());
        }
    };
    ...
    let default_cape_id =
        match DefaultMinecraftCape::get(profile.id, &state.pool).await {
            Ok(cape) => cape.map(|c| c.id),
            Err(e) => {
                tracing::error!("Failed to get default cape: {}", e);
                None
            }
        };
    ...
    tracing::debug!("Returning {} capes", profile.capes.len());
    Ok(capes)
}
...
pub async fn get_available_skins() -> crate::Result<Vec<Skin>> {
    tracing::debug!("get_available_skins called");
    ...
    let profile = match selected_credentials.online_profile().await {
        ...
        None => {
            tracing::debug!(
                "No online profile available, returning empty skins list"
            );
            return Ok(Vec::new());
        }
    };

    let current_skin = match profile.current_skin() {
        Ok(skin) => skin,
        Err(e) => {
            tracing::error!("Failed to get current skin: {}", e);
            return Ok(Vec::new());
        }
    };
