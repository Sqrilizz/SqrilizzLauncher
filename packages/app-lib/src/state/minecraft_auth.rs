...
impl Credentials {
    ...
    pub async fn maybe_online_profile(
        ...
        // [AR] Patch: Skip online profile fetch for offline accounts
        if self.account_type == AccountType::Pirate.as_lowercase_str()
            || self.account_type == AccountType::ElyBy.as_lowercase_str()
        {
...
async fn minecraft_profile(
    ...
    // [AR] Patch: Add fallback for broken JSON responses
    let mut profile = match serde_json::from_str::<MinecraftProfile>(&text) {
        Ok(p) => p,
        Err(source) => {
            tracing::warn!(
                "Failed to parse Minecraft profile, status: {}, body: {}",
                status,
                text
            );
